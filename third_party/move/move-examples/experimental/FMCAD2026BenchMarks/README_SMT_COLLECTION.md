...
From the `third_party/move/move-examples/experimental/FMCAD2026BenchMarks` directory, run:
```powershell
...
Note: `run_prove_and_collect_smt_v2.ps1` stops the prover early — once SMT files are detected the script terminates the prover process and collects the generated files. `console_output.txt` contains the captured stdout/stderr up to the point the prover was stopped.
