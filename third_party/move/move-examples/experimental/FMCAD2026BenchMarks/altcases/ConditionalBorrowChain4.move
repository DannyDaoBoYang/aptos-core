module BasicCoin::ProphecyBenchmark {
    ...
    // The "Parent Node" containing 4 distinct integer fields
    ...
    public fun update_one(node: Node, choice: u64): Node {
        ...
        let target = if (choice%4 == 0) { &mut n_ref.v0 }
        else if (choice%4 == 1) { &mut n_ref.v1 }
        else if (choice%4 == 2) { &mut n_ref.v2 }
        else { &mut n_ref.v3 };
        // The Mutation
        *target = *target + 1;
