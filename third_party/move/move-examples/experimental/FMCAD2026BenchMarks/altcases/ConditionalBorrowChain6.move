module BasicCoin::ProphecyBenchmark {
    ...
    // The "Parent Node" containing 6 distinct integer fields
    ...
    public fun new_node(): Node {
        Node { v0: 0, v1: 0, v2: 0, v3: 0, v4: 0, v5: 0 }
    }
    ...
    public fun update_one(node: Node, choice: u64): Node {
        ...
        let target = if (choice%6 == 0) { &mut n_ref.v0 }
        else if (choice%6 == 1) { &mut n_ref.v1 }
        else if (choice%6 == 2) { &mut n_ref.v2 }
        else if (choice%6 == 3) { &mut n_ref.v3 }
        else if (choice%6 == 4) { &mut n_ref.v4 }
        else { &mut n_ref.v5 };
        // The Mutation
        *target = *target + 1;
