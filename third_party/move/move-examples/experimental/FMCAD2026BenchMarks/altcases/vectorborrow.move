module BasicCoin::VerifyVector {
    ...
    fun verify_model_borrow_mut<Element>(v: &mut vector<Element>, i: u64): &mut Element {
        let returned_val = vector::borrow_mut(v, i);
        returned_val
    }
    spec verify_model_borrow_mut {
        aborts_if i >= len(v);
        ensures result == old(v[i]); // TODO: enough?
    }

    fun verify_model_borrow_mut_int(v: &mut vector<u64>, i: u64): &mut u64 {
        let returned_val = vector::borrow_mut(v, i);
        *returned_val = *returned_val + 1;
        returned_val
    }
    spec verify_model_borrow_mut_int {
        aborts_if i >= len(v);
        aborts_if v[i] == MAX_U64;
        ensures result == old(v[i])+1; // TODO: enough?
        ensures result == v[i];
    }
