module BasicCoin::ProphecyBenchmark {
    ...
    // The "Parent Node" containing 7 distinct integer fields
    struct Node has copy, drop {
        v0: u64, v1: u64, v2: u64, v3: u64, 
        v4: u64, v5: u64, v6: u64,
    }

    // Helper to create a clean node
    public fun new_node(): Node {
        Node { v0: 0, v1: 0, v2: 0, v3: 0, v4: 0, v5: 0, v6: 0 }
    }
    ...
    public fun update_one(node: Node, choice: u64): Node {
        ...
        let target = if (choice%7 == 0) { &mut n_ref.v0 }
        else if (choice%7 == 1) { &mut n_ref.v1 }
        else if (choice%7 == 2) { &mut n_ref.v2 }
        else if (choice%7 == 3) { &mut n_ref.v3 }
        else if (choice%7 == 4) { &mut n_ref.v4 }
        else if (choice%7 == 5) { &mut n_ref.v5 }
        else { &mut n_ref.v6 };
        // The Mutation
        *target = *target + 1;
	...
	requires node.v5 <= 100;
	requires node.v6 <= 100;
