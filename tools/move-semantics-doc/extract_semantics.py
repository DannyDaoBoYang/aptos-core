...
def generate_markdown(bytecodes):
    ...
    print()
    print("This document describes the formal semantics of Move bytecode instructions as implemented in the Move VM interpreter.")
    print()
    print("## Overview")
    print()
    print("The Move Virtual Machine (VM) executes Move bytecode through an interpreter that operates on a stack-based architecture. Each bytecode instruction has formally defined semantics that specify:")
    print()
    print("- **Behavior**: What the instruction does (described in pseudocode)")
    print("- **Stack effects**: How it manipulates the operand stack")
    print("- **Static operands**: Parameters encoded in the bytecode")
    print("- **Type safety**: Runtime checks ensuring type correctness")
    print()
    print("## Notation")
    print()
    print("The semantics use the following notation:")
    print()
    print("- `stack >> var` - Pop a value from the operand stack and bind it to `var`")
    print("- `stack << val` - Push `val` onto the operand stack")
    print("- `call_stack >> frame` - Pop a frame from the call stack")
    print("- `call_stack << frame` - Push a frame onto the call stack")
    print("- `current_frame.pc` - Program counter (instruction pointer) of the current frame")
    print("- `ty_stack` - Type stack used for runtime type checking (in paranoid mode)")
    print("- `(*ref)` - Dereference operation on a reference")
    print()
    print("## Instruction Groups")
    print()
    print("Bytecode instructions are organized into the following functional groups:")
    print()
    print("1. **Control Flow** - Branch, call, return, and abort instructions")
    print("2. **Stack and Local** - Stack manipulation and local variable access")
    print("3. **Reference** - Reference creation and manipulation")
    print("4. **Arithmetic** - Integer arithmetic operations")
    print("5. **Casting** - Type conversion between integer types")
    print("6. **Bitwise** - Bitwise logical operations")
    print("7. **Comparison** - Value comparison operations")
    print("8. **Boolean** - Boolean logical operations")
    print("9. **Struct** - Struct operations (pack, unpack, borrow field)")
    print("10. **Variant** - Enum variant operations")
    print("11. **Global** - Global storage operations (move_to, move_from, borrow_global)")
    print("12. **Vector** - Vector operations")
    print()
    print("---")
    print()
